...
import { EditAnnouncementBody } from '../../core/announcements/api-contract/edit-announcement/EditAnnouncementBody';
...
const EditAnnouncement: React.FC = () => {
    ...
    useEffect(() => {
        ...
        AnnouncementsGateway.GetAnnouncement(query['id'] as string)
            .then(announcement => {
                setLoading(false);
    ...
    return (
        <Layout>
            <PageHeaders title={'Modifier Annonce'} subTitle={'Annonces'} />
            ...
            <div className={'bg-white p-4 h-full w-full'}>
                ...
                {announcementRef.current && (
                    <EditAnnouncementForm announcement={announcementRef.current!} />
                )}
            </div>
        </Layout>
    );
...
const EditAnnouncementForm: React.FC<{ announcement: Announcement }> = ({ announcement }) => {
    const [loading, setLoading] = React.useState<boolean>(false);
    const [formValues, setFormValues] = React.useState<EditAnnouncementBody>(announcement);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await AnnouncementsGateway.EditAnnouncement(announcement.id, formValues);
